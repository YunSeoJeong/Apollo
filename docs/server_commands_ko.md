...
| Client Args | 클라이언트가 이 명령 뒤에 인수 문자열을 붙일 수 있게 허용합니다. |
...
`allow-client-args`의 기본값은 `false`입니다. 이 값을 특정 명령에서 켜지 않으면 기존 서버 명령 동작은 그대로 유지됩니다.
...
```text
payload[0]      = 서버 명령 인덱스 (uint8)
payload[1..end] = 선택 사항인 UTF-8 인수 문자열
```

2번 명령을 인수 없이 실행하려면 다음처럼 보냅니다.

```text
[0x02]
```

2번 명령에 인수를 붙여 실행하려면 다음처럼 보냅니다.

```text
[0x02] + "--profile living-room --fullscreen"
```

클라이언트 인수가 있고 해당 명령에서 인수를 허용한 경우, Apollo는 다음 형태로 실행합니다.
...
클라이언트 인수는 원본 명령줄 문자열 그대로 뒤에 붙습니다. 따라서 클라이언트는 호스트 플랫폼과 대상 명령이 기대하는 quoting 규칙에 맞춰 인수를 보내야 합니다.
